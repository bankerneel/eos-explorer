...
export class SupportComponent implements OnInit {
  ...
  constructor(private alaService: AlaService,
  ...
  loadTables() {
    ...
    this.alaService.ala.getTableRows(
      {
        json: true,
        code: this.contract,
        scope: this.contract,
  ...
  refund() {
    ...
    this.scatterService.refund().then(transaction => {
      timer(0, 3000).pipe(
        ...
        this.alaService.getDeferTransaction(transaction.transaction_id).subscribe(data => {
          if (data) {
            this.loadTables();
          }
        });
